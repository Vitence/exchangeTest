$("#reset").click(function(){
    var accountEle = $(".forgetPassMain").find('input').eq(0);
    var imgCodeEle = $(".forgetPassMain").find('input').eq(1);
    var tokenName = $("input[name='tokenName']").val();
    var token = $("input[name='token']").val();
    if(accountEle.val().trim() == ""){
        userErrorTwo(accountEle,'请输入邮箱');return;
    }
    if(imgCodeEle.val().trim() == ""){
        userErrorTwo(imgCodeEle,'请输入图片验证码');return;
    }
    $.post('/password/forget',{
        ...
        if(json.code =='0000'){
            ...
            userErrorTwo(imgCodeEle,'图片验证码错误');return;
        }else if(json.code == '1004'){
            userErrorTwo(accountEle,'邮箱未注册');return;
        }else if(json.code == '1002'){
            userErrorTwo(accountEle,'邮箱格式错误');return;
        }else{
            errMsg('邮件发送失败');
        }
    })
...
$("#set").click(function(){
    var passwordEle = $(".forgetPassMain").find('input').eq(0);
    var passwordAgainEle = $(".forgetPassMain").find('input').eq(1);
    var tokenName = $("input[name='tokenName']").val();
    var token = $("input[name='token']").val();
    if(passwordEle.val().trim() == ""){
        userErrorTwo(passwordEle,'请输入密码');return;
    }
    if(!checkPassword(passwordEle.val().trim())){
        userErrorTwo(passwordEle,'请填写8-16位字母加数字密码');return;
    }

    if(passwordEle.val().trim() != passwordAgainEle.val().trim()){
        userErrorTwo(passwordAgainEle,'两次密码输入不同');return;
    }
    $.post('/password/set',{
